...
/*Recupere les room et les stores dans une liste chainee dans ordre alphabetique*/
        /*si c est une command || comment*/
            /*!si cest une valide comment
                 /*stop parsing
            /*extraire la commande
            /*avance a la ligne suivante
/*      /* si c est une room et une valide room
            /*si connection part est actif
                /*return error
            /* creer la link list
            /*actif room_part
        /* si c est une connexion.
            /*si room part n est pas actif
                /*return error
            /*ative la connetion part
            /* ajoute les linbks dans la room approprie*/
t_lst_room *get_room_and_connection(int fd, t_lst_room *li)
{
    char *line;
    int part;  // 1 = room_part 2 = connexion part
    while(get_next_line(fd, &line))
    {
        if(!is_valid(line, part)) // verifie la validite de la line
            break;
        if (is_comment(line)) // checker le type de line et sa validite
        {
            get_next_line(fd, &line);
            command = extract_command(line); //extract command type 
        }
        else if (is_room(line)) // checker if line is a room et sa validite
        {
            create_list_room(line, li);
            part = 1;
        }
        else if (is_connexion(line))
        {
            part = 2;
            add_link_to_room(line, li);
        }
    }
    return(lst_room);
}
...
t_room *mapreader(int fd, t_room *room_tree)
{
    ...
    total_ants = get_ant_number(fd); //ERROR GERER DANS IS_NOT_ANT
    lst_room = get_room_and_connection(fd, lst_room);
